...
int main(void)
{
	...
	while (strlen(nombre) > 0) // Mientras la longitud de lo que tiene nombre sea mayor a cero, si no ponemos nombre, la longitud queda en cero.
	{
		...
		else
		{
			...
			ingresacadena(nombre, LNOM, "Nombre que quiero para el nodo"); // uso esto para volver a ingresar un nuevo nombre de haberlo, si lo hay vuelvo a entrar al While
		}
	}
	actual = inicio; //lo copio como temporal para no perderlo
	while (inicio != NULL) //mientras inicio no sea NULL y vamos a MOSTRAR LO QUE HAY EN EL NODO
	{
	...
	while (inicio != NULL) //aca vamos a LIBERAR LA LISTA
	{
		...
		flee(liberar); //libero memoria de liberar al sistema
	}
	system("pause");
	return 0;
}
